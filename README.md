...
`src/main.cpp (line 288)` by calling `Vehicle::PlanPath()` function. The function changes
the target lane and target speed parameters of the Controller instance.
...
`src/main.cpp (lines 290-293)`.

Examples of path planner in action:
...
defined in Road class. Road has three lanes left, middle and right with idtentifiers 1, 2 and 3.
...
way points has to be defined. First two way points W1 and W2 of the spline are determined by vehicle position in
...
simplify math and eliminating yaw angle `vehicle.cpp (lines 153-160)`. 
